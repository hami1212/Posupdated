...
class ContactUtil extends Util
{
    ...
    public function getContactQuery($business_id, $type, $contact_ids = [])
    {
        ...
        if (in_array($type, ['supplier', 'both'])) {
            $query->addSelect([
                DB::raw("SUM(IF(t.type = 'purchase', final_total, 0)) as total_purchase"),
                DB::raw("SUM(IF(t.type = 'purchase', (SELECT SUM(amount) FROM transaction_payments WHERE transaction_payments.transaction_id=t.id), 0)) as purchase_paid"),
                DB::raw("(SUM(IF(t.type = 'purchase', final_total, 0)) - SUM(IF(t.type = 'purchase', (SELECT SUM(amount) FROM transaction_payments WHERE transaction_payments.transaction_id=t.id), 0))) as purchase_due"),
                DB::raw("SUM(IF(t.type = 'purchase', (SELECT SUM(IF(is_return = 1, amount, 0)) FROM transaction_payments WHERE transaction_payments.transaction_id = t.id), 0)) as tr"),
                DB::raw("SUM(IF(t.type = 'purchase_return', final_total, 0)) as total_purchase_return"),
                DB::raw("SUM(IF(t.type = 'purchase_return', (SELECT SUM(amount) FROM transaction_payments WHERE transaction_payments.transaction_id=t.id), 0)) as purchase_return_paid")
            ]);
        }
    ...
    DB::raw("SUM(IF(t.type = 'sell' AND t.status = 'final', final_total, 0)) as total_invoice"),
                DB::raw("SUM(IF(t.type = 'sell' AND t.status = 'final', (SELECT SUM(IF(is_return = 1,-1*amount,amount)) FROM transaction_payments WHERE transaction_payments.transaction_id=t.id), 0)) as invoice_received"),
                DB::raw("(SUM(IF(t.type = 'sell' AND t.status = 'final', final_total, 0)) - SUM(IF(t.type = 'sell' AND t.status = 'final', (SELECT SUM(IF(is_return = 1, -1*amount, amount)) FROM transaction_payments WHERE transaction_payments.transaction_id=t.id), 0))) as difference"),
                DB::raw("SUM(IF(t.type = 'sell' AND t.status = 'final', (SELECT SUM(IF(is_return = 1, amount, 0)) FROM transaction_payments WHERE transaction_payments.transaction_id = t.id), 0)) as tr"),
                DB::raw("SUM(IF(t.type = 'sell_return', final_total, 0)) as total_sell_return"),
                DB::raw("SUM(IF(t.type = 'sell_return', (SELECT SUM(amount) FROM transaction_payments WHERE transaction_payments.transaction_id=t.id), 0)) as sell_return_paid")
            ]);
        }

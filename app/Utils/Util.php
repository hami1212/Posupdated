...
class Util
{
    ...
    public function getContactDue($contact_id, $business_id = null)
    {
        $query = Contact::where('contacts.id', $contact_id)
            ...
            ->whereIn('t.type', ['sell', 'sell_return','opening_balance', 'purchase'])
            ->select(
                ...
                DB::raw("SUM(IF(t.type = 'opening_balance', (SELECT SUM(amount) FROM transaction_payments WHERE transaction_payments.transaction_id=t.id), 0)) as opening_balance_paid"),
                 DB::raw("SUM(IF(t.type = 'sell_return', final_total, 0)) as sell_return")
            );
        if (!empty($business_id)) {
        ...
        $due = $contact_payments->total_invoice + $contact_payments->total_purchase - $contact_payments->total_paid - $contact_payments->purchase_paid + $contact_payments->opening_balance - $contact_payments->opening_balance_paid - $contact_payments->sell_return;
    ...
    {
        $quantity_precision = session('constants.quantity_precision', 2);

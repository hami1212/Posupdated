...
class ReportController extends Controller
{
    ...
    public function getproductSellReport(Request $request)
    {
        ...
        if ($request->ajax()) {
            ...
            $query = TransactionSellLine::join(
                ...
                ->join('contacts as c', 't.contact_id', '=', 'c.id')
                ->join('products as p', 'pv.product_id', '=', 'p.id')
                 ->join('users', 'p.created_by', '=', 'users.id')
    ...
    //saad
    public function productSellReportByday(Request $request)
    {
        ...
        if ($request->ajax()) {
            $query = TransactionSellLine::join(
                ...
                ->where('t.status', 'final')
                ->select(
                    'transaction_sell_lines.id as transaction_id',
                    't.transaction_date',
                    DB::raw('DATE_FORMAT(t.transaction_date, "%Y-%m-%d") as formated_date'),
                    DB::raw('SUM((transaction_sell_lines.quantity - transaction_sell_lines.quantity_returned) * transaction_sell_lines.unit_price_inc_tax) as subtotal'),
                    DB::raw('DAYNAME(t.transaction_date) as day_name'), // Add the day name column
                    DB::raw('SUM(transaction_sell_lines.quantity - transaction_sell_lines.quantity_returned) as total_qty_sold'),
                )
                ->groupBy('formated_date');
     ...
     public function user_login(Request $request)
    {
        ...
        $ur = auth()->user()->business_id;
        $business_id = $request->session()->get('user.business_id');
        if (request()->ajax()) {

            $data =  DB::table('activity_log')
                ->join('users', 'users.id', '=', 'activity_log.causer_id')
                ->join('business', 'business.id', '=', 'activity_log.business_id')
                ->select(
                    'users.username',
                    'business.name as b_name',
                    \DB::raw(" MAX(activity_log.created_at) AS created_at")
                )
                ->where('activity_log.description', '=', 'login')
                ->groupby('activity_log.business_id');

            if (!empty($request->input('business_id'))) {
                $business_id = $request->business_id;
                $data->where('business.id', '=', $business_id);
            }
            if (!empty($request->input('user_id'))) {
                $user_id = $request->user_id;
        ...
        return view('report.userlogin')->with(compact('users', 'business'));
    }
